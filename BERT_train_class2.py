...
k_fold = StratifiedShuffleSplit(n_splits=5, random_state=0)
for train, test in k_fold.split(X, y):
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            predicted_labels = torch.round(torch.softmax(outputs))
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(test_loader):
                ...
                test_outputs.extend(torch.softmax(outputs).cpu().detach().numpy().tolist())
